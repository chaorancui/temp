...
在多头注意力机制（Multi-Head Attention）中，`Q`、`K` 和 `V` 的维度会进一步被分割成多个“头”。假设模型的输入维度是 $ d_{\text{model}} $，而有 $ h $ 个头（多头注意力的数量），则每个头的 `Q`、`K`、`V` 向量的维度一般为：

$$ \text{Each head's dimension} = \frac{d_{\text{model}}}{h} $$

这意味着，每个头都会处理一个更小的维度，计算并捕捉不同的上下文信息。最后，多头注意力机制会将每个头的输出拼接起来，并通过线性变换将其映射回模型的原始维度 $ d_{\text{model}} $。
假设模型输入的维度为 $ d_{\text{model}} = 512 $，并且有 8 个头的多头注意力机制。那么：
