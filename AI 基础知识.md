...
2. **常见的 Embedding 实现算子**

   Embedding 通常用一个简单的**查表（lookup table）**操作实现。查表的原理是将输入的索引（如词汇 ID）对应到预先训练好的嵌入矩阵中的某一行。这一行向量就是该词的向量表示。

   在 PyTorch 中，**`torch.nn.Embedding`** 是最常用的 Embedding 层实现。它能够将输入的词汇 ID 映射到嵌入向量。

   **例子**：

   假设我们有一个词汇表大小为`10000`，每个词的嵌入向量维度为`300`。我们使用 PyTorch 来实现词嵌入：

   ```python
   import torch
   import torch.nn as nn

   # 设定词汇表大小为10000，嵌入维度为300
   embedding_layer = nn.Embedding(num_embeddings=10000, embedding_dim=300)

   # 假设我们有一个句子，它的词汇ID表示为[1, 23, 456]
   input_ids = torch.tensor([1, 23, 456])

   # 使用embedding层将词汇ID转化为向量表示
   embedded_output = embedding_layer(input_ids)

   print(embedded_output.shape)  # 输出的shape应该是(3, 300)，表示每个词对应一个300维的向量
   ```

   在这个例子中，`nn.Embedding`创建了一个嵌入层，将输入的词汇 ID `[1, 23, 456]` 映射为相应的词向量。每个词汇 ID 对应的向量维度为`300`。
