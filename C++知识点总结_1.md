...
C++中的宏函数展开是一个重要的预处理特性。我来介绍主要使用方法和规则:
...
1. 最外层宏首先被**识别**
2. 但其**参数会先被完全展开**（除非遇到`#`或`##`）
3. 然后才会进行宏体替换
4. 最后替换后的结果被重新扫描以查找更多宏
...
// 展开过程：
// 1. **首先识别最外层宏**：编译器首先看到`SQUARE(DOUBLE(3))`，识别`SQUARE`是宏
// 2. **展开参数前先扫描参数中的宏**：在将参数`DOUBLE(3)`替换到`SQUARE`之前，需要先完全展开这个参数，即展开`DOUBLE(3)` → `(2 * (3))`
// 3. **将展开后的参数代入主宏**：现在有`SQUARE((2 * (3)))`
// 4. **展开主宏**：`(( (2 * (3)) ) * ( (2 * (3)) ))`
// 5. **最终结果**：`((2 * (3)) * (2 * (3)))`
```
...
根据 C++标准（和 C 标准类似）的预处理阶段规则：

1. **参数扫描**：当宏被识别时，其参数会先被完全展开（除非该参数在宏体中是字符串化`#`或标记粘贴`##`的操作数）
2. **替换**：展开的参数被替换到宏体中
3. **重扫描**：替换后的结果会被重新扫描以查找更多宏
...
A(C)
// 展开过程：
// 1. 识别 A(C)，需要先展开参数 C → 123
// 2. 现在有 A(123)，展开为 B(123)
// 3. 重新扫描得到 B(123)，展开为[123]
```
...
int xy = 10;
STR(DOUBLE(3))  // 结果是 "DOUBLE(3)" 而不是 "(2*(3))"
CONCAT(x,y)     // 结果是 xy，值为10
```

### 转换宏或两层宏

如果希望在使用 `#` 或 `##` 之前，参数已经完全展开，就需要定义转换宏。如下：
...
1. 带有 `#` 的情况：
   ...
   ------------------------------------------------
   // 正确的做法：通过中间宏实现展开
   #define STR_IMPL(x) #x        // 带#的实现宏
   #define STR(x) STR_IMPL(x)    // 展开用的转换宏

   #define A 100
   STR(A)  // 现在结果是 "100"
   // 展开过程：
   // 1. STR(A) -> STR_IMPL(100)
   // 2. STR_IMPL(100) -> "100"
   ```
...
2. 带有 `##` 的情况：
   ...
   CONCAT(PREFIX, SUFFIX)  // 结果是 "fooSUFFIX" 而不是 "foo123"
   ----------------------------------------------------------------
   // 同理对于##
   #define CONCAT_IMPL(a, b) a##b    // 带##的实现宏
   #define CONCAT(a, b) CONCAT_IMPL(a, b)  // 展开用的转换宏

   #define PREFIX foo
   #define SUFFIX 123

   CONCAT(PREFIX, SUFFIX)  // 现在结果是 "foo123"
   // 展开过程：
   // 1. CONCAT(PREFIX, SUFFIX) -> CONCAT_IMPL(foo, 123)
   // 2. CONCAT_IMPL(foo, 123) -> foo123
   ```

「转换宏」或「两层宏」的结构在涉及**字符串化**或**拼接**时是**非常推荐的规范**写法。
...
1. 可变参数宏:
   ...
   // 使用示例
   DEBUG_LOG("Value is %d", x);
   ```

2. 多语句宏：使用 do-while(0) 包裹
   ...
   **问题示例：**
   ...
   ```cpp
   // 问题示例：
   #define INIT() init_a(); init_b();  // 多语句宏
   ...
   // 宏展开后：
   if (condition)
       init_a(); init_b();;  // 展开成两条独立语句！
   else
       do_something();
   ```

   - `init_b();` 变成了一条独立语句，**不再属于 `if` 作用域**！
   - 由于 `init_b();` 脱离了 `if`，编译器会认为 `else` 没有对应的 `if`，**编译报错**。
   - 由于**用户习惯性加分号**，添加 `{}` 大括号如 `#define INIT() { init_a(); init_b(); }` 仍然可能导致**`else` 无法匹配 `if`**。

   **解决方案：do-while(0)**

   ```cpp
   #define INIT() do { init_a(); init_b(); } while(0)
   ...
   // 宏展开后：
   if (condition)
       do { init_a(); init_b(); } while(0);  // 整体是一条语句！
   else
       do_something();
   ```

   - `do-while(0)` **是一个完整的语句**，后面的 `;` 不会影响 `if-else` 结构。

### 注意

1. 宏参数在展开前会先展开所有参数
2. 运算符优先级问题,需要合理使用括号
3. 字符串化操作符(#)和连接操作符(##)不能同时对同一个参数使用
4. 递归宏是不允许的
5. 预处理器不理解 C++语法,只进行文本替换

使用宏时应当谨慎,过度使用可能导致代码难以维护。在现代 C++中,建议优先使用内联函数、constexpr 等特性。仅在确实需要预处理器功能时才使用宏。
...
// 多个宏 或 要用如下写法。#ifdef 只能跟一个宏标识符，后面的被当做无害代码。
#if defined(DEBUG_1) || defined(DEBUG_2) || defined(DEBUG_3) // 正确写法
#ifdef DEBUG_1 || DEBUG_2 || DEBUG_3 // 错误写法
```
