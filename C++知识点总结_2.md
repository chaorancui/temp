...
在 C++ 中，`constexpr int`（非静态成员）和 `static constexpr int`（静态成员）的核心区别在于**存储归属**和**值是否共享**。具体可以从以下 4 个维度来剖析：

1. **存储归属与内存布局（最核心区别）**
   - **非静态 `constexpr int`**：属于**对象实例**。每个类对象都拥有一份独立的该成员内存（虽然作为常量通常会被优化掉，但从语法和对象模型上讲，它影响 `sizeof(类)`）。
   - **静态 `static constexpr int`**：属于**类本身**。全局只有一份副本，所有对象共享该值，且**不占用对象的内存空间**（即不影响 `sizeof(类)`）。

   ```cpp
   struct S {
       constexpr int a = 10;        // 非静态，影响对象大小
       static constexpr int b = 20; // 静态，不影响对象大小
   };

   static_assert(sizeof(S) == sizeof(int)); // 通常成立（对齐忽略），因为 b 不占空间
   ```

2. **值的可变性（是否可因对象而异）**
   - **非静态成员**：每个对象可以拥有不同的值。你可以在构造函数初始化列表（甚至 `constexpr` 构造函数）中对其进行初始化。
   - **静态成员**：全局唯一，所有对象的值**必须完全相同**。

   ```cpp
   struct Foo {
       constexpr int x;
       constexpr Foo(int v) : x(v) {} // 合法：不同对象可传不同值
   };

   constexpr Foo f1(10), f2(20);
   // f1.x == 10, f2.x == 20，各不同
   ```

3. **C++17 前后的链接/定义规则（极易踩坑）**
   - **C++17 之前**：
     - **非静态**：直接在类内初始化即可，无需类外定义（它作为默认成员初始化器存在）。
     - **静态**：如果在代码中**取地址**或**绑定到引用**（即 ODR-use），则**必须**在类外提供一个单独的定义（通常放在 `.cpp` 文件），否则会引发链接错误。

   - **C++17 及之后**：
     - `static constexpr` 成员**隐式成为 `inline` 变量**。这意味着即使取地址，也**无需**在类外额外定义，直接使用即可，极大简化了代码。
   ...
   // C++17 之前：若执行以下操作，必须取消下面这行注释（提供定义）
   // constexpr int Demo::value;

   // C++17 之后：上面那行定义完全不需要，编译器会处理
   int main() {
       const int* p = &Demo::value; // C++17 合法，C++14 若没有类外定义则链接报错
   }
   ```

4. 使用场景与权限
   - **非静态 `constexpr`**：通常用于那些虽然初始化值固定，但在不同对象中可能依赖构造函数参数的场景（例如类模板中传入不同值）。
   - **静态 `static constexpr`**：通常用于定义**类级别的常量**，比如固定的配置参数、数学常数（`static constexpr double PI = 3.14;`）或枚举替代品，且支持在编译期直接作为模板参数使用。

**总结速查表**

| 对比维度               | `constexpr int` (非静态)     | `static constexpr int` (静态) |
| ---------------------- | ---------------------------- | ----------------------------- |
| **内存归属**           | 每个对象一份                 | 类全局唯一一份                |
| **影响 `sizeof`**      | **是**（通常增加 4 字节）    | **否**                        |
| **值是否可因对象不同** | **可以**（通过构造函数指定） | **不可以**（全局固定）        |
| **类外定义 (C++17前)** | 不需要                       | ODR-use 时需要                |
| **类外定义 (C++17后)** | 不需要                       | 完全不需要（隐式 inline）     |
| **典型用途**           | 对象内部不变量               | 全局类常量、枚举替代          |

**选型建议**：

- 如果该常量是类级别的"通用属性"（如最大长度、版本号），请用 `static constexpr` 以节省内存且语义清晰；
- 如果该常量在不同对象实例中可能不同，则用非静态 `constexpr`。
