...
在 Linux 中，“用户（user）”和“组（group）”是权限模型的两大核心概念。它们共同构成了典型多用户系统下的访问控制基础。下面按**概念 → 差异 → 权限判定机制 → 多用户登录的典型管理方式**的逻辑进行系统说明。

**一、用户（User）**

1. 定义

   用户是**系统中执行操作的主体**。每一个进程在运行时，都会绑定一个“有效用户身份”。

   在内核层面，用户由 **UID（User ID）** 唯一标识：

   - `0`：root（超级用户）
   - `1–999`（或 `1–1000`）：系统/服务用户（发行版相关）
   - `>=1000`：普通登录用户
...
1. 定义
   ...
   组是**用户的集合**，用于**批量授权和权限管理**。

   内核层面由 **GID（Group ID）** 唯一标识。
...
> 用户代表“谁在做事”，组代表“这批人能做什么”。

**四、Linux 权限模型如何使用用户和组**
...
1. 文件/目录的三元权限模型
   ...
   1. 如果进程的 **euid == 文件 owner**
      → 使用 owner 权限
   2. 否则如果进程的 **egid ∈ 文件 group**
      → 使用 group 权限
   3. 否则
      → 使用 other 权限
...
1. 普通多用户互不干扰（默认模型）
   ...
   - 目录权限：`700` 或 `750`
   - 用户彼此不可访问对方目录
   - 每人一个私有主组（user-private-group 模型）
...
`su` 是 **substitute user** 或 **switch user** 的缩写，用于在当前会话中**切换到另一个用户身份**，默认是切换到 `root` 用户。

```bash
su [options] [username]
```

- 如果 **不指定 username**，默认切换到 `root`
- 切换后，会获得目标用户的权限
- 若要退出回话，输入 `exit` 即可。
...
- `su` 会启动一个 **新的 shell**
- 该 shell 的 **有效 UID / GID** 变为目标用户
- **需要输入目标用户的密码**
  - 普通用户 → root：需要 root 密码
  - root → 普通用户：通常不需要密码

这也是 `su` 与 `sudo` 在安全模型上的一个核心差异。

**三、使用 `su` 切换用户的常见方式**

1. 切换到 root 用户（不加载完整环境）
   ...
   - 只切换 **身份**
   - **环境变量基本保持当前用户**
   - `PATH` 可能不包含 `/sbin`、`/usr/sbin`

   适合：临时执行一些 root 操作

2. 切换到 root 用户（登录 shell，推荐）
   ...
   - 加载 `/etc/profile`
   - 加载 `~/.bash_profile` / `~/.profile`
   - 使用 root 的 `HOME`、`PATH`、`USER`

   这是**最标准、最安全的 root 切换方式**。
...
| 对比项   | su                          | sudo                    |
| -------- | --------------------------- | ----------------------- |
| 认证方式 | 目标用户密码（通常是 root） | 当前用户密码            |
| 权限控制 | 粗粒度（全切换）            | 细粒度（命令级）        |
| 审计能力 | 较弱                        | 强（/var/log/auth.log） |
| 使用场景 | 用户切换、救援环境          | 日常运维、生产环境      |
| 安全性   | root 密码暴露风险           | 更安全、可控            |
...
3. 普通用户无法使用 `su`
   ...
   - 用户不在 `wheel` / `sudo` 相关安全策略中
   - PAM 限制（如 `/etc/pam.d/su`）

   典型配置（只允许 wheel 组使用 su）：
