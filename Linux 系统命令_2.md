...
在 Linux 中，删除由 `ln` 命令创建的链接（无论是**软链接**还是**硬链接**），最常用、最安全的方法是使用 **`rm`** 命令。

另外，针对软链接，还可以使用 **`unlink`** 命令。

1. **核心删除命令及效果**
   - **方法 A：使用 `rm` 命令（最常用）**

     `rm`（remove）是最通用的删除命令，既可以删文件，也可以删链接。
     - **删除软链接：**
     ...
     - **删除硬链接：**
       ...
       - 效果： 删除了该硬链接。因为硬链接和源文件本质上指向同一个底层数据块（inode），只有当指向该 inode 的所有硬链接（包括源文件本身）都被删除时，文件内容才会真正被系统回收。只要还有一个硬链接存在，数据就不会丢失。

   - **方法 B：使用 `unlink` 命令**

     `unlink` 是一个更底层的系统调用工具，只能用于删除**单个文件或单个链接**，不能删除目录。
     - **删除软/硬链接：**
       ...
       - 效果： 与 `rm` 类似，只删除链接本身，不影响源文件。它的好处是"极简"，不支持任何参数（如 `-r` 或 `-f`），因此操作非常安全，绝不会误删整个目录。

2. **致命陷阱：关于"指向目录的软链接"**

   如果你创建了一个指向**目录**的软链接，在删除它时**带斜杠和不带斜杠**效果有差异。
   - 错误示范：带斜杠 `rm -rf dir_link/`

     ```bash
     rm -rf dir_link/
     ```

     - **危险效果：** 注意最后的斜杠 `/`！加上斜杠后，Linux 会把 `dir_link/` 识别为**源目录本身**。这条命令会**把源目录下的所有真实文件全部清空**，而软链接本身可能还会残留在那里。

   - 正确做法：不带斜杠 `rm dir_link`

     ```bash
     rm dir_link
     ```

     - **正确效果：** 干净利落地只删除这个软链接，源目录和里面的文件安然无恙。

     > 安全小贴士：
     > 如果你要删除指向目录的软链接，强烈建议使用 `unlink dir_link`。因为 `unlink` 不允许带斜杠，哪怕你输入 `unlink dir_link/`，系统也会直接报错拒绝执行，从而完美规避误删源目录的风险。

3. **不同命令效果对比总结**

   | **删除命令**            | **作用于软链接 (Link)** | **作用于硬链接 (Hard Link)** | **作用于指向目录的软链接**                                | **危险指数 / 备注** |
   | ----------------------- | ----------------------- | ---------------------------- | --------------------------------------------------------- | ------------------- |
   | **`rm link_name`**      | 仅删链接，源文件保留    | 仅删当前硬链接，不影响其他   | 仅删链接（**千万别加斜杠 `/`**）                          | ⭐ （最常用）       |
   | **`unlink link_name`**  | 仅删链接，源文件保留    | 仅删当前硬链接，不影响其他   | 仅删链接（不支持斜杠，最安全）                            | （最安全，防误删）  |
   | **`rm -rf link_name/`** | ------                  | ------                       | **毁灭性：** 链接没删掉，但**清空了源目录下的所有文件**！ |
