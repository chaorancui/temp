...
1. 最基本用法（**必须加 `-O` 或 `-o`**）
   ...
   ```bash
   curl -O https://example.com/files/data.tar.gz
   ```
...
2. 指定保存文件名
   ...
   ```bash
   curl -o my_data.tar.gz https://example.com/files/data.tar.gz
   ```
...
3. 显示下载进度（推荐）
   ...
   ```bash
   curl -L -O https://example.com/files/data.tar.gz
   ```

   - `-L`：**跟随重定向**（非常关键，很多网页下载都需要）

4. 断点续传

   ```bash
   curl -C - -O https://example.com/files/data.tar.gz
   ```
...
6. HTTPS 证书问题（内网常见）
   ...
   curl -k -O https://example.com/data.tar.gz
   ```

## wget 命令

使用 `wget` 下载压缩包（推荐，语义清晰）

1. 最基本用法

   ```bash
   wget https://example.com/files/data.tar.gz
   ```
...
2. 指定保存文件名
   ...
   ```bash
   wget -O my_data.tar.gz https://example.com/files/data.tar.gz
   ```
...
3. 后台下载（大文件常用）
   ...
   ```bash
   wget -b https://example.com/files/data.zip
   ```
   ...
   ```bash
   wget -c https://example.com/files/data.tar.gz
   ```
...
4. 需要登录 / Cookie 的网页
   ...
   wget \
     ...
     https://example.com/private/data.tar.gz
   ```
...
2. 实际返回的是 HTML，而不是压缩包

   常见原因：未登录，或者 Cookie / token 缺失。

   检查方法：`file data.zip`，如果看到：`HTML document text`，说明下载的是网页，不是压缩包。
