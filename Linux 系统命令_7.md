...
| 算法   | 输出长度      | 安全性   | 速度            | 是否已被破解 | 典型用途               | 命令                 |
| ------ | ------------- | -------- | --------------- | ------------ | ---------------------- | -------------------- |
| MD5    | 128 bit       | 很低     | 很快            | 已碰撞破解   | 文件完整性、非安全场景 | `md5sum file.bin`    |
| SHA1   | 160 bit       | 较低     | 快              | 已碰撞破解   | Git（历史）、旧系统    | `sha1sum file.bin`   |
| SHA256 | 256 bit       | 高       | 中等            | 目前安全     | 安全校验、发布包       | `sha256sum file.bin` |
| SHA512 | 512 bit       | 很高     | 较快（64位CPU） | 目前安全     | 高安全场景             | `sha512sum file.bin` |
| BLAKE2 | 256/512 bit   | 高       | 很快            | 目前安全     | 工程推荐               | `b2sum file.bin`     |
| BLAKE3 | 可变          | 很高     | 极快            | 目前安全     | 大文件、高性能场景     | `b3sum file.bin`     |
| CRC32  | 32 bit        | 无安全性 | 极快            | 很容易冲突   | 传输校验               | `crc32 file.bin`     |
| xxHash | 32/64/128 bit | 无安全性 | 极快            | 非加密 hash  | 缓存、去重             | `xxhsum file.bin`    |

**注意**：下面通常不是系统内置命令

- BLAKE3 需安装 `sudo apt install b3sum`
- CRC32 需安装 `sudo apt install libarchive-zip-perl`
- xxHash 需安装 `sudo apt install xxhash`

**建议**：

- 即使只是“完整性校验”，也建议直接 `SHA256`, 不要再使用 MD5/SHA1。因为成本几乎没差，工具完全成熟。
- 大文件可优先考虑 BLAKE3。如AI 模型、ONNX、checkpoint等
- 分布式系统推荐 `BLAKE2 / BLAKE3`。因为CPU 更省、不容易 IO 瓶颈、hash 开销明显下降。

**性能大致排序**：

```bash
CRC32 > xxHash > BLAKE3 > BLAKE2 > MD5 > SHA1 > SHA256 > SHA512
```

但 SHA512 在 x86_64 上可能比 SHA256 快，真正瓶颈经常是磁盘 IO，而不是 hash 算法。
...
3. **SHA256（最主流）**
   ...
   **特点**
   ...
   - 安全性目前足够
     ...
     相比 MD5，更安全，更慢一些。但现代 CPU 上通常已经够快。
...
4. **SHA512**
   ...
   **特点**
   - 更长摘要
   - 在 64 位 CPU 上经常比 SHA256 更快

     很多人误以为 SHA512 一定更慢，其实未必。
...
6. **BLAKE3（现代高性能方案）**
   ...
   **问题**
   ...
   Linux 默认通常没有自带命令。需要安装：`sudo apt install b3sum`
...
2. **场景二：安全相关**，推荐 `SHA256 / SHA512 / BLAKE2`
