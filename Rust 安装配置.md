...
1. 基础文件管理
   ...
   在进入 Yazi 界面后，你可以使用以下快捷键进行增删改查：

   | **操作**            | **快捷键**     | **备注**                              |
   | ------------------- | -------------- | ------------------------------------- |
   | **浏览/进入目录**   | `l` 或 `Enter` | 向右进入文件夹或打开文件              |
   | **返回上级目录**    | `h`            | 向左退回父目录                        |
   | **新建文件/文件夹** | `a` (append)   | 输入名字即可；若以 `/` 结尾则创建目录 |
   | **删除文件**        | `d`            | 通常会先放入回收站                    |
   | **重命名**          | `r`            | 快速重构文件名                        |
   | **拷贝 (Copy)**     | `y`            | 复制选中的文件到剪贴板                |
   | **剪切 (Cut)**      | `x`            | 移动选中的文件到剪贴板                |
   | **粘贴 (Paste)**    | `p`            | 将剪贴板内容放置在当前目录            |

2. 批量操作与搜索

   Yazi 的强大之处在于其高效的批量处理能力：
   - **多选文件**：使用 `Space`（空格）选中单个文件，或者使用 `v` 进入可视模式（Visual Mode）进行连选。
   - **搜索文件**：按下 `/` 即可在当前目录下进行增量搜索。
   - **全局搜索**：按下 `s` (grep) 可以在当前目录及其子目录中搜索文件内容（需要安装 `ripgrep`）。

3. 进阶常用功能

   除了基本的增删改查，这些操作能让你起飞：
   - **筛选 (Filter)**：按 `f` 输入字符，实时过滤当前窗口显示的文件，这在处理包含数百个文件的文件夹时极度好用。
   - **预览控制**：Yazi 默认会预览代码、图像甚至 PDF。你可以使用 `Ctrl-u` / `Ctrl-d` 在预览窗口中上下翻页。
   - **打开方式**：按 `Enter` 默认在 Neovim 中打开，但如果你按 `o`，可以选择用系统默认程序或其他工具打开。
   - **跳转到路径**：按 `g` 然后接快捷键（如 `g` `h` 跳到 Home，`g` `c` 跳到配置目录）。

4. LazyVim 特有的交互

   在 LazyVim 环境中，通常你会通过以下方式唤起 Yazi：
   - **打开 Yazi**：通常绑定在 `<leader>f-something`，如果你使用的是标准的 `yazi.nvim` 配置，通常是 `<leader>fy` 或你自定义的快捷键。
   - **同步目录**：当你通过 Yazi 打开一个文件时，Neovim 的当前工作目录（CWD）通常会自动同步。

   > [!TIP]
   > **小贴士**：如果你在 Yazi 中误删了文件，通常它被移到了系统的回收站（Trash）而不是直接抹除，你可以去系统回收站找回。
