...
Vim 括号操作速查表（`i` = inside，`a` = around）

| 操作类型           | 语法  | 说明                            | 示例效果（以 `abc[123]xyz` 为例） |
| ------------------ | ----- | ------------------------------- | --------------------------------- |
| **选中括号内内容** | `vi[` | Visual 选中 `[` 与 `]` 内的内容 | 选中 `123`                        |
| **选中括号和内容** | `va[` | Visual 选中整个 `[123]`         | 选中 `[123]`                      |
| **删除括号内内容** | `di[` | 删除方括号内内容，保留括号      | → `abc[]xyz`                      |
| **删除括号和内容** | `da[` | 删除整个括号对及内容            | → `abcxyz`                        |
| **复制括号内内容** | `yi[` | 复制 `[...]` 内的内容           | yank `123`                        |
| **复制括号和内容** | `ya[` | 复制整个 `[123]`                | yank `[123]`                      |
| **修改括号内内容** | `ci[` | 删除括号内内容并进入插入模式    | 光标停在 `[` 与 `]` 之间          |
| **修改括号和内容** | `ca[` | 删除括号+内容并进入插入模式     | 光标停在原 `[123]` 位置           |

### Vim 括号/引号替换与包裹操作速查表

Vim 括号/引号替换与包裹操作速查表（基于 **vim-surround**，Vim / Neovim 常用增强插件之一）。

1. **添加包裹**

   在 **可视模式** 选中一段文本后（`v` 或 `V`），输入：

   | 操作                | 命令      | 示例                     |
   | ------------------- | --------- | ------------------------ |
   | 用 `"` 包裹选中内容 | `S"`      | 选中 `foo` → 变 `"foo"`  |
   | 用 `'` 包裹         | `S'`      | `foo` → `'foo'`          |
   | 用 `()` 包裹        | `S)`      | `foo` → `(foo)`          |
   | 用 `[]` 包裹        | `S]`      | `foo` → `[foo]`          |
   | 用 `{}` 包裹        | `S}`      | `foo` → `{foo}`          |
   | 用 `<div>` 包裹     | `St<div>` | `foo` → `<div>foo</div>` |

   > 💡 提示：`S` 来自 _vim-surround_，在选中模式下代表“用什么符号包裹选区”。

2. **删除包裹**

   光标放在被包裹的区域内（例如 `"foo"` 中的 `foo` 上），执行：
...
3. **替换包裹**
   ...
   | 将 `[foo]` 改为 `{foo}`           | `cs[{` |      |
   | 将 `'foo'` 改为 `"foo"`           | `cs'"` |      |
   | 将 `<tag>foo</tag>` 改为 `"` 包裹 | `cst"` |      |

   > 说明：
   >
   > - `c` = change
   > - `s` = surround
   > - `cs"` 表示：change surround from `"` to `'`

4. **插入模式下快速包裹**

   当你在插入模式中，比如输入完 `foo` 后想立刻包起来，可以用：

   | 操作           | 命令      | 示例            |
   | -------------- | --------- | --------------- |
   | 当前单词加引号 | `<C-g>s"` | `foo` → `"foo"` |
   | 当前单词加括号 | `<C-g>s)` | `foo` → `(foo)` |
...
5. **实战速查小结**
   ...
   | 目的                     | 命令        | 说明                |
   | ------------------------ | ----------- | ------------------- |
   | 删除外层引号             | `ds"`       | delete surround     |
   | 改变引号类型             | `cs"'`      | change surround     |
   | 给选中内容加括号         | 选中后 `S)` | surround selection  |
   | 给单词加括号（插入模式） | `<C-g>s)`   |                     |
   | 用 `<div>` 标签包裹      | `St<div>`   | 自动闭合成 `</div>` |

6. **启用 `vim-surround`**

   如果你的 Vim 没有这个功能，说明还没装插件：
...
系统整理 `diw` / `daw` 这一类 **Vim 文本对象（Text Objects）** 的用法。适用于 **Vim / Neovim / LazyVim**，不依赖任何插件。

**一、核心理念：Operator + Text Object**

Vim 中大量高效编辑操作都遵循以下范式：
...
| ------ | ------------------------- | ------ |
| `d`    | delete 删除               | `diw`  |
| `c`    | change 删除并进入插入模式 | `ciw`  |
| `y`    | yank 复制                 | `yiw`  |
| `v`    | 可视选择                  | `viw`  |
| `gU`   | 转大写                    | `gUiw` |
| `gu`   | 转小写                    | `guiw` |

**二、单词 / 句子 / 段落级文本对象（word / sentence / paragraph）**

1. word（单词）

   | 文本对象 | 含义                       | 是否包含空格 |
   | -------- | -------------------------- | ------------ |
   | `iw`     | inner word（单词本体）     | ❌ 不包含    |
   | `aw`     | around word（单词 + 空格） | ✅ 包含      |
   ...
   - **`w` 与 `W` 文本对象**
     ...
     diw   " 删除 `bar_baz`（视光标位置）
     diW   " 删除整个 `foo-bar_baz`
     ```
...
> **Vim 的效率来自：用“语义单位”操作文本，而不是用光标挪动字符。**

只要熟练 `operator + i/a + object`，你就已经进入 Vim 的“高阶编辑区”。
