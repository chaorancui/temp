...
在 LazyVim 中，让 Yazi 显示隐藏文件非常简单，有两种常用方法可以实现：

1. **方法一：快捷键（最快最常用）**

   在 Yazi 窗口中，直接按 **`.`（句号）** 键即可立刻切换隐藏文件的显示状态，这是最高效的方法。

2. **方法二：修改配置文件（永久生效）**

   如果你希望每次打开 Yazi 都自动显示隐藏文件，可以修改配置文件。
   1. **定位配置文件**：找到 Yazi 的配置文件 `yazi.toml`。对于 LazyVim，通常位于 `~/.config/yazi/` 目录下。

   2. **编辑配置文件**：使用 LazyVim 或其他文本编辑器打开 `yazi.toml` 文件。如果文件不存在，可以新建一个。

   3. **添加配置项**：在 `yazi.toml` 文件中，添加以下内容：
      ...
      > 这里的 `show_hidden` 项就控制了是否默认显示隐藏文件。

   修改并保存配置文件后，下次再打开 Yazi 时，隐藏文件就会直接显示出来。
