...
在 LazyVim（以及 Neovim）中，提取正则匹配内容并放到新 buffer 的操作，最经典且高效的方法是结合 **`:global`** 命令与 **寄存器**。这是 Vim 的传统强项。

思路是：清空一个寄存器 -> 将匹配行存入寄存器 -> 在新 buffer 中粘贴。

1. **清空寄存器**（以寄存器 `a` 为例）： 在 Normal 模式下输入 `qaq`（或者直接输入命令行 `:let @a=''`）。
...
2. **正则提取并追加**： 输入以下命令：
   ...
   > 注意：使用大写的 `A` 表示**追加**（Append），这样才不会被下一条匹配覆盖。

3. **拷贝到临时 Buffer**：
   - 输入 `:new` 或 `:vnew` 或 `:enew` 打开一个新的临时窗口。
   - 输入 `"ap` 将寄存器 `a` 的内容粘贴出来。

详解：

1. 寄存器（Register）的清空与追加技巧

   在 Vim 中，寄存器就像是多个独立的剪贴板。
   - **清空寄存器：**
     - 最快的方法（Normal 模式）： 输入 `qaq`。
       - 原理：`q` 是开始录制宏，`a` 是指定寄存器 `a`，第二个 `q` 是立即停止录制。这会把一个空的操作序列存入 `a`，从而清空它。
     - **命令行方法：** `:let @a = ""`。
   - **追加内容（大小写的奥秘）：**
     - 当你使用小写字母（如 `"ay`）时，Vim 会**覆盖**寄存器 `a` 的内容。
     - 当你使用大写字母（如 `"Ay`）时，Vim 会将内容**追加**到寄存器 `a` 的末尾。
     - **核心原理：** 在 `:g/regex/y A` 命令中，大写的 `A` 是保证你能提取出所有匹配行而不只是最后一行的关键。

2. `:global` 命令的底层原理

   `:global`（简写为 `:g`）的运行逻辑分为两个阶段：
   1. **第一遍扫描（标记）：** Vim 扫描整个文件（或指定范围），寻找匹配正则的行，并给这些行打上一个隐形的标记。
   2. **第二遍执行（执行）：** Vim 依次跳转到所有被标记的行，并执行你给出的命令（如 `y` 复制、`d` 删除、`s` 替换）。

   **命令拆解：** `:[range]g/pattern/[cmd]`
   - `range`：如果不写，默认是 `%`（整个文件）。
   - `pattern`：你的正则表达式。
   - `cmd`：对匹配行执行的 Ex 命令。如果不写，默认是 `:p`（打印内容）。

3. 如何编写复杂的正则表达式

   Vim 的正则默认比较“老旧”（很多符号需要反斜杠转义）。为了像在 Python 或 JavaScript 中那样写正则，强烈建议开启 **Very Magic** 模式。
   - **极度幻术模式 (`\v`)：** 在正则开头加上 `\v`，之后除了字母、数字和下划线，所有字符都有特殊含义，无需频繁转义。
     - **普通模式：** `/\(foo\)\|\(bar\)/`（太痛苦了）
     - **Very Magic：** `/\v(foo|bar)/`（非常直观）

   **复杂正则示例：** 如果你想提取所有含有 IP 地址的行： `/\v(\d{1,3}\.){3}\d{1,3}/`

   > **提示：** 在 LazyVim 中，你可以先用 `/` 进行搜索预览，确认高亮范围对了，再按 `Esc` 然后输入 `:g//...`。
   > 原理：如果 `:g` 后面留空 `//`，它会自动复用上一次搜索的正则。

4. 如何新建临时窗口（Scratch Buffer）

   当你提取完内容到寄存器后，你需要一个地方存放它。
   - **新建窗口命令：**
     - `:new`：水平新建窗口。
     - `:vnew`：垂直新建窗口。
     - `:enew`：在当前窗口新建一个空白 buffer。

   - **设置为“临时”属性：** Vim 默认会尝试把你新建的 buffer 当作一个真实文件。如果你希望它是临时的（关闭时不提示保存），可以在新 buffer 中输入：

     ```vim
     :set buftype=nofile
     ```

   - **快速粘贴：** 在新窗口中按 `"ap`（将寄存器 `a` 的内容粘贴出来）。
...
**使用 `snacks.nvim` 的搜索 (推荐)**

> 新版 LazyVim（v14.0+）确实已经不再默认使用 Telescope 了，现在默认的“全能插件”是 `snacks.nvim`。

- 按下 `<space> f f` 打开文件搜索。
- 按下 **`Ctrl + o p`** 粘贴复制内容 (在 Neovim 的命令行或输入框中粘贴剪贴板内容)。
- `snacks.nvim` 会自动过滤出该文件，回车打开。

如果你发现搜不到文件，但文件确实存在，通常有以下几种原因：

1. 默认排除规则 (Git & Hidden)

   Snacks.picker 的 `files` 默认会尊重 `.gitignore`。如果你粘贴的路径在被忽略的文件夹里，它会直接“隐身”。
   - **快捷键尝试：**
     - `<space> f f` 是搜索普通文件。
     - **`<space> f F`** (大写的 F) 是搜索**所有文件**（包括被 git 忽略的和隐藏文件）。
   - **交互式切换：** 在搜索窗口打开时，尝试按下 **`Ctrl + i`**。在 Snacks 中，这通常用于切换是否显示被忽略的文件（Ignored）。

2. 根目录检测 (Root Detection)

   Snacks 基于当前工作目录进行搜索的。如果你在一个大型项目的子目录下打开了 Neovim，它可能只在子目录里搜。
   - **现象：** 你的状态栏左侧通常会显示当前的 Root。
   - **解决：** 使用 **`<space> f p`** (Projects) 切换到正确的项目根目录，或者使用 **`<space> .`** (Find Files in CWD) 强制从当前目录搜索。

3. `fd` 缺失或配置冲突

   Snacks.picker 默认会优先调用 `fd` (或 `fdfind`)。如果你的系统中 `fd` 有全局配置（比如 `~/.config/fd/ignore`），Snacks 会受其影响。
   - **检查：** 在终端输入 `fd --version`。如果没有，它会回退到原生 Lua 实现，效率和过滤逻辑会有细微差别。
   - **特殊路径：** 如果你的路径包含特殊的字符（比如 `[` `]` 或大量的 `.`），模糊匹配引擎可能会产生歧义。

4. 绝对路径 vs 相对路径

   这是一个“冷知识”：Snacks 的 `files` picker 主要是为了**模糊搜索**设计的。如果你粘贴的是一个以 `/` 开头的绝对路径，它有时会尝试将其作为“搜索模式”而非“直接路径”来处理。
   - **验证：** 试着**删掉路径前缀**，只保留最后两级目录和文件名，看能不能搜到。

## 分屏操作

在 LazyVim 里管理分屏，主要是结合 Vim 原生 `<C-w>` 和 LazyVim 自己扩展的 `<Space>` 键两大类快捷键。

| 操作               | 快捷键 (Keymap)                                 | 备注                                                                  |
| :----------------- | :---------------------------------------------- | :-------------------------------------------------------------------- |
| **新建分屏**       |                                                 |                                                                       |
| 水平分割 (上下)    | `<Space>ws` 或 `<C-w>s`                         | 在当前窗口上方创建一个新的水平分屏。                                  |
| 垂直分割 (左右)    | `<Space>wv` 或 `<C-w>v`                         | 在当前窗口右侧创建一个新的垂直分屏。                                  |
| **切换分屏**       |                                                 |                                                                       |
| 方向键导航         | `<C-h>` `<C-j>` `<C-k>` `<C-l>`                 | 分别**切换到**左、下、上、右的窗口，这是 LazyVim 首选的快速导航方式。 |
| Vim 标准导航       | `<C-w>h` `<C-w>j` `<C-w>k` `<C-w>l`             | 作用同上，是 Vim 通用的导航方法。                                     |
| 循环切换窗口       | `<C-w>w`                                        | 在打开的窗口间循环切换焦点。                                          |
| Leader 键导航      | `<Space>wh` `<Space>wj` `<Space>wk` `<Space>wl` | 同样是切换到左、下、上、右的窗口。                                    |
| **退出分屏**       |                                                 |                                                                       |
| 关闭当前窗口       | `<C-w>q` 或 `:q`                                | 命令退出当前激活的分屏窗口。                                          |
| 保留当前，关闭其他 | `<Space>wo`                                     | 关闭除了当前窗口之外的所有其他分屏，使当前窗口最大化。                |

> `<C-w>` 代表 `Ctrl` + `w` 键。

另外，你还可以通过 `<Space>w=` 来让所有分屏窗口的大小恢复平均分配。
