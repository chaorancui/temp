...
这是个非常重要的问题！在使用切片（Slicing）时，**是否会改变张量的维度**，取决于你用的是：

- **标量索引（`x[i]`）**
- **切片对象（冒号 `:`）或整形数组、布尔索引、`None`、`...` 等高级索引**

重点记忆：

> **凡是使用单个整数索引的位置，维度就会减少。** > **使用 `:`、`start:stop`、`start:stop:step` 等切片对象，不会改变维度。**
...
- `b,c,d,o,x,X | e,E,f,F,g,G | s` 类型说明符：
  二进制，Unicode 字符，十进制（默认），八进制，十六进制（小写），十六进制（大写），
  科学计数法（小写 e）， 科学计数法（大写 E），定点表示法，同 f，但大写无穷/NaN，通用格式（自动选择 e/f），同 g，但大写 E/INF/NAN，
  字符串（默认），
