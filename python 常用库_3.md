...
高级索引（Fancy Indexing）是指使用**整数数组/列表/张量**作为索引，一次性从数组中选取任意位置的元素。NumPy 和 PyTorch 都支持这种索引方式。它与基本索引（切片、整数、`...`）最大的区别是：**高级索引返回的是数据的副本，而不是视图**，并且可以按照任意顺序、重复选取元素。

下面以 NumPy 为例（PyTorch 用法完全相同），从一维到高维逐步说明。

### 一维数组的高级索引

对于一维数组 `arr`，使用一维索引数组 `idx` 会返回一个新的一维数组，其元素按 `idx` 中的顺序从 `arr` 中取出，新数组的形状与 `idx` 相同（`idx` 可以是高维，后面会看到）。

```python
import numpy as np

arr = np.array([10, 20, 30, 40, 50])
idx = [3, 1, 2, 1]      # 可以重复，可以乱序
result = arr[idx]
print(result)           # [40 20 30 20]
print(result.shape)     # (4,)
```

- `idx` 中的每个值必须在 `[0, len(arr)-1]` 范围内，否则报错。
- 返回的数组形状与 `idx` 的形状**相同**。例如若 `idx` 是 2×3 的数组，结果也是 2×3。

```python
idx_2d = np.array([[0, 1], [2, 3]])   # 形状 (2,2)
result = arr[idx_2d]
print(result)
# [[10 20]
#  [30 40]]
...
二维数组 `arr` 形状 `(M, N)`。高级索引可以作用于行、列或同时作用。

1. **只对行进行高级索引**

   使用一维索引数组 `row_idx`，结果形状为 `(len(row_idx), N)`。相当于按 `row_idx` 指定的顺序选取行。

   ```python
   arr = np.arange(12).reshape(4, 3)   # 4行3列
   row_idx = [2, 0, 2]
   result = arr[row_idx]
...
2. **同时对行和列进行高级索引**
   ...
   提供两个形状**相同**的索引数组 `row_idx` 和 `col_idx`，结果会返回一个一维数组，其中每个元素是 `arr[row_idx[i], col_idx[i]]`。结果的形状等于 `row_idx.shape`（或 `col_idx.shape`）。

   ```python
   arr = np.arange(12).reshape(4, 3)
   row_idx = np.array([0, 2, 1, 2])
   col_idx = np.array([1, 0, 2, 1])
   result = arr[row_idx, col_idx]
   print(result)          # [1 6 5 7]  对应位置：(0,1),(2,0),(1,2),(2,1)
   print(result.shape)    # (4,)
   ```

   如果 `row_idx` 和 `col_idx` 是更高维的，结果形状也保持该维度。

   ```python
   row_idx = np.array([[0,1],[2,0]])   # (2,2)
   col_idx = np.array([[1,2],[0,1]])   # (2,2)
   result = arr[row_idx, col_idx]
   print(result)
   # [[1 5]
   #  [6 1]]
...
3. **广播机制**
   ...
   当两个索引数组的形状不一致时，NumPy 会尝试**广播**到相同形状，然后再配对。

   ```python
   arr = np.arange(12).reshape(4,3)
   row_idx = np.array([0, 1, 2])        # 形状 (3,)
   col_idx = np.array([0])              # 形状 (1,)
   result = arr[row_idx, col_idx]       # col_idx 广播到 (3,)
   print(result)   # [0 3 6]  即 (0,0), (1,0), (2,0)
   ```

   更复杂的广播示例：

   ```python
   row_idx = np.array([[0], [2]])       # (2,1)
   col_idx = np.array([1, 2])           # (2,)
   # row_idx 广播到 (2,2), col_idx 广播到 (2,2)
   result = arr[row_idx, col_idx]
   print(result)
   # [[1 2]
   #  [7 8]]
...
4. **高级索引与切片、整数索引混合**
   ...
   这是最容易困惑的地方。当同时存在**高级索引**和**基本索引**（切片、整数）时，规则是：
   - 基本索引用于指定维度，高级索引会**替代**其对应维度，并产生"交叉组合"的效果。

   - 最终结果的维度顺序：**高级索引对应的维度放在前面，基本索引对应的维度放在后面**。

   例如：`arr[rows, cols]` 只有高级索引 → 结果是一维（或与索引形状相同）。
   `arr[rows, :, cols]` 这种混合，需要谨慎。

   **常见例子**：

   ```python
   arr = np.arange(24).reshape(4, 2, 3)   # 形状 (4,2,3)
   row_idx = np.array([0, 1])             # 形状 (2,)
   col_idx = np.array([2, 0])             # 形状 (2,)

   # 情况1：arr[row_idx, :, col_idx]
   # 规则：第0维用row_idx，第1维是切片(:)，第2维用col_idx
   # 结果形状 = row_idx.shape + (2,) = (2, 2)
   result = arr[row_idx, :, col_idx]
   print(result.shape)   # (2, 2)
   ```

   **规则详细说明**（来自 NumPy 文档）：
   1. 基本索引（切片、整数）控制某些维度不参与高级索引的"花式"操作。
   2. 高级索引（整数数组）一起广播，它们的形状会合并成结果的前几个维度。
   3. 基本索引所对应的维度会追加到结果维度的末尾（保持它们在原数组中的相对顺序）。
   4. 如果用了整数作为索引（不是数组），该维度会被压缩掉。
   ...
   idx = np.array([0,1,2])
   # arr[idx, :, :, idx]   -> 形状 (3, 20, 30) ??? 不对，实际要看 idx 出现在哪些维度
   # 更清晰：arr[:, idx, :, idx]  形状 (10, 3, 30)  因为第二个和第四个是高级索引，会合并为 (3,) 放在前面，然后是基本维度（第0维10，第2维30）
   ```
   ...
   arr = np.zeros((10,20,30,40))
   row_idx = np.array([0,1,2])   # 形状 (3,)
   col_idx = np.array([0,1,2])   # 形状 (3,)
   # 注意：arr[:, row_idx, :, col_idx]
   # 高级索引维度：row_idx 和 col_idx 广播后形状 (3,)
   # 基本维度：第0维（10）和第2维（30）
   # 最终形状 = (3, 10, 30)
   result = arr[:, row_idx, :, col_idx]
   print(result.shape)   # (3, 10, 30)
   ```
...
- 单独对一个轴使用高级索引，该轴被替换为索引数组的形状。

- 同时对多个轴使用高级索引，这些索引数组会广播，结果形状是广播后的形状 + 剩余基本索引维度的形状。

**示例**：三维数组 (3,4,5) 分别对三个轴做高级索引

```python
arr = np.random.rand(3,4,5)
i = np.array([0,2])
j = np.array([1,1,3])   # 形状(3,)
k = np.array([2,4])

# arr[i, j, k] 需要 i,j,k 广播到相同形状
# i: (2,) -> 广播到(3,2)? 实际广播规则：缺失维度补1，然后扩展。i形状(2,), j(3,), k(2,) -> 不能直接广播，形状不兼容。需要手动 reshape 或使用 np.ix_
# 正确做法：使用 np.ix_ 生成索引网格
i_grid, j_grid, k_grid = np.ix_(i, j, k)  # 分别形状 (2,1,1), (1,3,1), (1,1,2)
result = arr[i_grid, j_grid, k_grid]      # 形状 (2,3,2)
```

实际常用 `np.ix_` 来得到所有交叉组合。

### 注意事项与总结

| 特性                   | 基本索引（切片/整数）      | 高级索引（整数数组）       |
| ---------------------- | -------------------------- | -------------------------- |
| 返回视图还是副本       | 视图（切片）、降维（整数） | **总是副本**               |
| 是否允许重复/乱序      | 不允许（切片是有序连续）   | 允许任意顺序和重复         |
| 结果形状               | 取决于切片和整数           | 取决于索引数组的形状及广播 |
| 是否支持广播           | 不涉及                     | 支持索引数组之间的广播     |
| 与切片混合时的维度顺序 | 高级索引维度在前，基本在后 | 同上                       |
