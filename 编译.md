...
> [1]. [GDB 官方文档 | 主页](http://sourceware.org/gdb/current/onlinedocs/gdb/index.html) > [1]. [GDB 官方文档 | 命令脚本](https://sourceware.org/gdb/current/onlinedocs/gdb.html/Command-Files.html)
...
1. **自定义命令格式**
   ...
   其中 `<code>` 可以是任意 gdb 命令。此外自定义命令**还支持最多 10 个输入参数**：`$arg0，$arg1 …… $arg9`，并且还用 `$argc` 来标明一共传入了多少参数。**最新版本中支持的参数个数是不受限**。

   `<help text>` 是此命令的帮助信息。不知道为什么，只显示一行。
...
1. `-U`
   ...
   - **用途**：用于从缓存中移除指定的变量。
   - **示例**：`cmake -U var_name ..`
   - **说明**：此选项可以帮助清除不再需要的变量，避免在重新配置时引入过期的值。

2. `-D`

   - **用途**：用于定义或设置 CMake 变量。
   - **示例**：`cmake -DVAR_NAME=VALUE ..`
   - **说明**：通过这个选项，可以在命令行中指定 CMake 变量的值，从而影响构建配置。

3. `-S`

   - **用途**：指定源代码的路径。
   - **示例**：`cmake -S /path/to/source`
   - **说明**：该选项用来告诉 CMake 源代码的位置，以便它可以读取 `CMakeLists.txt` 文件。

4. `-B`

   - **用途**：指定构建目录。
   - **示例**：`cmake -B /path/to/build`
   - **说明**：通过这个选项，可以将构建输出放在指定的目录中，这样可以避免与源代码混合。

示例

```bash
cmake -S /path/to/source -B /path/to/build -DVAR_NAME=VALUE -UOLD_VAR
```

这个命令会在指定的构建目录中生成构建文件，设置变量 `VAR_NAME` 的值，并移除缓存中名为 `OLD_VAR` 的变量。
