...
在 **gdb 调试时被卡在函数死循环**，可以用下面的方法 **立刻中断执行并查看调用栈**。

1. **使用 Ctrl + C 中断程序**

   在 gdb 运行程序时（执行了 `run`、`continue` 或触发死循环），按：

   ```bash
   Ctrl + C
   ```

   gdb 会立刻停止当前执行，并返回一个 prompt，例如：

   ```bash
   Program received signal SIGINT, Interrupt.
   0x00005555555551bc in foo_loop() at foo.cpp:42
   ```
...
3. **如果 Ctrl+C 没反应**
   ...
   有些情况下进程可能被系统抢占或陷入内核态，Ctrl+C 反应很慢，可以用：

   ```bash
   (gdb) interrupt
   ```

4. **如果程序在远程调试（gdbserver）**

   死循环发生在远端目标上时，本地 gdb 里也可以：

   ```bash
   Ctrl + C
   ```

   如果仍然不中断，可以强制向 gdbserver 发送 SIGINT：

   ```bash
   (gdb) monitor interrupt
   ```
