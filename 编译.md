...
4. 安全考虑：
   默认情况下，GDB 可能会禁用自动加载本地`.gdbinit`文件（当前目录下的）以防安全风险。如果遇到这种情况，你可以在用户主目录的`.gdbinit`中添加以下行来启用它：
...
GDB 自定义命令确实不能直接使用 for 或 while 循环语句，因为 GDB 命令语言不支持这些控制结构。但是，我们可以通过其他方式来实现循环打印数组中的值。以下是几种方法：

1. 使用 `while` 命令：

GDB 提供了一个 `while` 命令，可以重复执行一系列命令直到条件不满足。这是最接近传统循环的方法。
...
使用方法：`print_array 10`（假设数组有 10 个元素）
